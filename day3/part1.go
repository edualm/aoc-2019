...
func test() error {
	result := run("R2,U2,L2,D2\nL2,U3,R3,D2")

	if result != 3 {
		return fmt.Errorf("result should equal 3, got %d instead, bailing out", result)
	}

	result = run("R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83")

	if result != 159 {
		return fmt.Errorf("result should equal 159, got %d instead, bailing out", result)
	}

	result = run("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51\nU98,R91,D20,R16,D67,R40,U7,R15,U6,R7")

	if result != 135 {
		return fmt.Errorf("result should equal 135, got %d instead, bailing out", result)
	}
...
func lowestManhattan(p []point) int {
	lowest := math.MaxInt64

	for i := 0; i < len(p); i++ {
		calculated := manhattan(p[i])
...
func run(data string) int {
	...
	return lowestManhattan(intersection)
}
