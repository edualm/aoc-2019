...
public class Computer {
    ...
    private var waitForInput: Bool
    
    public var nextInput: Int? {
        get {
            guard inputs.count > 0 else {
                return waitForInput ? nil : -1
            }
    ...
    public init(program: [Int], waitForInput: Bool = true) {
        self.state = program
        self.waitForInput = waitForInput
    }
    
    private init(state: [Int], outputs: [Int], instructionPointer: Int, relativeBase: Int, nextInput: Int?, waitForInput: Bool = true) {
        self.state = state
    ...
    @discardableResult public func run() -> ComputationState {
        while true {
            let cs = compute()
    ...
    @discardableResult public func runSingle() -> ComputationState {
        let cs = compute()
